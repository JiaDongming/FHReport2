...
$(function () {
    ...
    $("#btnSaveAsImage").click(function () {
        var chart = $("#divGlobalReportHTML117").getKendoChart();
...
function createChart(strCharts) {
    $("#divGlobalReportHTML117").kendoChart({
        dataSource: {
            data: eval(strCharts)
        },
        title: {
...
function doReportCall() {
    ...
    $.ajax({
        type: "POST",
        url: "Report17.aspx/RefreshGlobalReport17",
        data: params,
        contentType: "application/json; charset=utf-8",
        beforeSend: function (XMLHttpRequest) {
            $('#divGlobalReportHTML117').html("<h5 style='color:red;'>正在生成报表...</h5>");
        },
        success: function (msg) {
            $('#divGlobalReportHTML117').html("");
            if (msg.d) {
                var strCharts = msg.d.toString();
                $("#divGlobalReportHTML117").ready(createChart(strCharts));
                $("#divGlobalReportHTML117").bind("kendo:skinChange", createChart(strCharts));
            }
        },
