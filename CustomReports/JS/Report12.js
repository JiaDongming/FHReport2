...
$(function () {
    ...
    $("#btnSaveAsImage").click(function () {
        var chart = $("#divGlobalReportHTML112").getKendoChart();
...
function createChart(strCharts) {
    $("#divGlobalReportHTML112").kendoChart({
        dataSource: {
            data: eval(strCharts)
        },
        title: {
...
function doReportCall() {
    ...
    $.ajax({
        type: "POST",
        url: "Report12.aspx/RefreshGlobalReport12",
        data: params,
        contentType: "application/json; charset=utf-8",
        beforeSend: function (XMLHttpRequest) {
            $('#divGlobalReportHTML112').html("<h5 style='color:red;'>正在生成报表...</h5>");
        },
        success: function (msg) {
            $('#divGlobalReportHTML112').html("");
            if (msg.d) {
                var strCharts = msg.d.toString();
                $("#divGlobalReportHTML112").ready(createChart(strCharts));
                $("#divGlobalReportHTML112").bind("kendo:skinChange", createChart(strCharts));
            }
        },
