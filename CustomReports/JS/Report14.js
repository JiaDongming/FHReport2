...
$(function () {
    ...
    $("#btnSaveAsImage").click(function () {
        var chart = $("#divGlobalReportHTML114").getKendoChart();
...
function createChart(strCharts) {
    $("#divGlobalReportHTML114").kendoChart({
        dataSource: {
            data: eval(strCharts)
        },
        title: {
...
function doReportCall() {
    ...
    $.ajax({
        type: "POST",
        url: "Report14.aspx/RefreshGlobalReport14",
        data: params,
        contentType: "application/json; charset=utf-8",
        beforeSend: function (XMLHttpRequest) {
            $('#divGlobalReportHTML114').html("<h5 style='color:red;'>正在生成报表...</h5>");
        },
        success: function (msg) {
            $('#divGlobalReportHTML114').html("");
            if (msg.d) {
                var strCharts = msg.d.toString();
                $("#divGlobalReportHTML114").ready(createChart(strCharts));
                $("#divGlobalReportHTML114").bind("kendo:skinChange", createChart(strCharts));
            }
        },
